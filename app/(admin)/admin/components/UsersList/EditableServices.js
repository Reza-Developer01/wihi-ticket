...
const EditableServices = ({ allServices = [], selected = [], onChange }) => {
  ...
  // انتخاب یا حذف سرویس از selected
            ...
            <ul className="divide-y divide-[#EFF0F6] text-sm text-[#8C8C8C]">
              {services.length ? (
                services.map((item, index) => (
                  ...
                  >
                    <span onClick={() => toggleService(item)}>{item.name}</span>
                    <svg
